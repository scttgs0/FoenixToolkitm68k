using System;
using System.IO;

using Gtk;
using GUI = Gtk.Builder.ObjectAttribute;

using FoenixCore;
using static FoenixCore.Simulator.FileFormat.ResourceChecker;


namespace FoenixToolkit.UI
{
    class AssetWindow : Window
    {
        public static AssetWindow Instance;
        private FoenixSystem _kernel;
        private int _rowCount = 0;

#pragma warning disable CS0169  // never assigned
        //[GUI] Button btnAdd;
        //[GUI] TreeView treeAssets;
#pragma warning restore CS0649

        public AssetWindow() : this(new Builder("AssetWindow.ui")) { }

        private AssetWindow(Builder builder) : base(builder.GetRawOwnedObject("AssetWindow"))
        {
            builder.Autoconnect(this);

            Instance = this;
            HideOnDelete();
        }

        public void SetKernel(FoenixSystem krnl)
        {
            _kernel = krnl;
            _rowCount = _kernel.Resources.Items.Count;
        }

        public void UpdateAssets()
        {
            _rowCount = _kernel.Resources.Items.Count;
        }

        private void AssetGrid_CellValueNeeded(object sender, EventArgs e)
        {
            //-- try
            // {
            //     Resource res = _kernel.Resources.Items[e.RowIndex];

            //     switch (e.ColumnIndex)
            //     {
            //         case 0:
            //             e.Value = res.Name;
            //             break;

            //         case 1:
            //             e.Value = res.StartAddress.ToString("X6");
            //             break;

            //         case 2:
            //             e.Value = (res.StartAddress + res.Length - 1).ToString("X6");
            //             break;

            //         case 3:
            //             e.Value = res.FileType.ToString();
            //             break;
            //     }
            // }
            // catch
            // {
            //     // whatever!
            // }
        }

        private void on_btnAdd_clicked(object sender, EventArgs e)
        {
            //-- MainWindow.Instance.LoadImageToolStripMenuItem_Click(this, null);
        }

        private void AssetGrid_CellClick(object sender, EventArgs e)
        {
            //-- // Get the resource for the RowIndex
            // if (e.RowIndex > -1)
            // {
            //     Resource res = _kernel.Resources.Items[e.RowIndex];

            //     switch (e.ColumnIndex)
            //     {
            //         // Export the asset to file
            //         case 4:
            //             FileInfo info = new FileInfo(res.Name);
            //             //-- SaveFileDialog saveDlg = new SaveFileDialog
            //             // {
            //             //     Title = "Save Asset to File",
            //             //     Filter = "Raw File (*.bin)|*.bin|Palette File (*.pal)|*.pal |Tilemap File (*.tls)| *.tls",
            //             //     FileName = info.Name
            //             // };

            //             // if (saveDlg.ShowDialog() == DialogResult.OK)
            //             // {
            //             //     FileStream dataFile = File.Create(saveDlg.FileName, 0x800, FileOptions.SequentialScan);
            //             //     byte[] buffer = new byte[res.Length];
            //             //     _kernel.MemMgr.CopyIntoBuffer(res.StartAddress, res.Length, buffer);
            //             //     dataFile.Write(buffer, 0, res.Length);
            //             //     dataFile.Close();
            //             // }
            //             break;

            //         // Browse this page in the Memory Window
            //         case 5:
            //             MemoryWindow.Instance.GotoAddress(res.StartAddress & 0xFFFF00);
            //             break;

            //         // Delete the row, but copy the values into our input boxes
            //         case 6:
            //             _kernel.Resources.Items.Remove(res);

            //             // zero the data at the location
            //             byte[] zeroes = new byte[res.Length];
            //             _kernel.MemMgr.CopyBuffer(zeroes, 0, res.StartAddress, res.Length);
            //             --_rowCount;
            //             break;
            //     }
            // }
        }
    }
}
